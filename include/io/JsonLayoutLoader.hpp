...
#include <rules/MinSpacingRule.hpp>  // added for JSON parsing
#include <rules/IntersectionRule.hpp>  // allow intersection rules from JSON
...
public:
    std::shared_ptr<Layout> load(const std::string& filename) override
    {
        ...
        else if (layoutJson["layers"].is_object())
        {
            ...
            for (auto it = layoutJson["layers"].begin(); it != layoutJson["layers"].end(); ++it)
            {
                ...
                if (props.contains("minWidth")) cfg.minWidth = props["minWidth"].get<double>();
                if (props.contains("minHeight")) cfg.minHeight = props["minHeight"].get<double>();
                if (props.contains("minArea")) cfg.minArea = props["minArea"].get<double>();
    ...
    std::pair<std::shared_ptr<Layout>, std::vector<DrcRulePtr>> loadWithRules(const std::string& filename)
    {
        ...
        json layoutJson;
        try { file >> layoutJson; } catch (const json::exception& e) { throw std::runtime_error(std::string("Failed to parse JSON: ") + e.what()); }
        file.close();
        ...
        if (layoutJson.contains("shapes") && layoutJson["shapes"].is_array())
        {
            for (const auto& sj : layoutJson["shapes"]) {
                ...
                if (!sj.contains("layer") || !sj.contains("rect")) continue;
                std::string lname = sj["layer"].get<std::string>();
                auto lid = layout->getLayerIdByName(lname);
                if (lid < 0) continue;
                Shape s = parseShape(sj, lid, 0);
                Layer* layerPtr = const_cast<Layer*>(layout->getLayer(lid));
                if (layerPtr) layerPtr->addShape(s);
            }
        }
...
private:
    ...
    Rect parseRect(const json& rectJson)
    {
        ...
        {
            throw std::runtime_error(
                "Rectangle bounds invalid: min must be less than max");
        }
