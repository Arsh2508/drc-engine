...
public:
    EnclosureRule(const std::string& innerLayerName,
                  ...
                  double minMargin)
        : DrcRule("Enclosure"), m_inner(innerLayerName), m_outer(outerLayerName), m_minMargin(minMargin)
    {
    }
    ...
    {
        return "Enclosure: " + m_inner + " in " + m_outer + " margin=" + std::to_string(m_minMargin);
    }
